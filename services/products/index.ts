    ...
    try {
        ...
        if(formData.details.hasVariants) {
            ...
            for(let variant of formData.details.variants) {
                ...
                const _variant = await pocketbase.collection('ProductVariants').create({ options: _options, stock: variant.stock, price: variant.price, allowBackorder: variant.allowBackOrder, manageInventory: variant.manageInventory, media: [...variant.media], thumb: variant.thumb === null ? (variant.media.length > 0 ? variant.media[0] : null ) : variant.thumb });
            ...
            const defaultVariant = await pocketbase.collection('ProductVariants').create({ options: [], stock: formData.details.defaultVariant.stock, price: formData.details.defaultVariant.price, allowBackorder: formData.details.defaultVariant.allowBackOrder, manageInventory: formData.details.defaultVariant.manageInventory, media: [...formData.details.media], thumb:formData.details.thumb === null ? ( formData.details.media.length > 0 ? formData.details.media[0] : null  ) : formData.details.thumb });
