<template>
    <div
        class="flex flex-col h-[100dvh] w-screen overflow-y-scroll overflow-x-scroll lg:p-5 items-center justify-center  bg-black/10">
        <div class="bg-white md:border md:shadow h-full md:h-fit md:rounded-lg w-full max-w-[500px] divide-y">
            ...
            <div class="flex flex-col">
                <div class="flex flex-row w-full p-3 items-center gap-3">
                    <div class="h-12 aspect-square border bg-center bg-cover rounded-full grid place-items-center"
                        :style="{ backgroundImage: `url('${createObjectUrl(this.signUp.avatar)}')` }">
                        <span class="font-medium text-black/50 text-lg" v-if="signUp.avatar === null">{{ signUp.name[0]
                            }}</span>
                    </div>
                    <div class="flex flex-row gap-1 items-center">
                        <button :disabled="signUp.avatar !== null" @click="addImage"
                            class="text-sm font-medium px-4 p-1 border rounded disabled:opacity-50">Upload</button>
                        <button :disabled="signUp.avatar === null" @click="signUp.avatar = null"
                            class="text-sm font-medium px-4 p-1 border rounded disabled:opacity-50">Remove</button>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col">
                <div class="flex flex-col p-3">
                    ...
                    <div class="border-2 flex flex-row gap-2 rounded-lg focus-within:border-primary">
                        ...
                        <input :type="signUp.passwordVisible ? 'text' : 'password'"
                            class="p-2 focus:outline-none w-full rounded-lg" v-model="v$.signUp.password.$model"
                            placeholder="***********" />

                        <button class="p-2" @click="signUp.passwordVisible = !signUp.passwordVisible">
                            <PhosphorIconEye v-if="signUp.passwordVisible" :size="20" />
                ...
                <div class="flex flex-col p-3">
                    ...
                    <div class="border-2 flex flex-row gap-2 rounded-lg focus-within:border-primary">
                        ...
                        <input :type="signUp.passwordConfirmVisible ? 'text' : 'password'"
                            class="p-2 focus:outline-none w-full rounded-lg" v-model="v$.signUp.passwordConfirm.$model"
                            placeholder="***********" />

                        <button class="p-2" @click="signUp.passwordConfirmVisible = !signUp.passwordConfirmVisible">
                            <PhosphorIconEye v-if="signUp.passwordConfirmVisible" :size="20" />
                            <PhosphorIconEyeSlash v-else :size="20" />
                        </button>
                    </div>
            ...
            <div class="flex flex-col w-full p-3">
                <button @click="signUpAdmin" :disabled="signUp.loading"
