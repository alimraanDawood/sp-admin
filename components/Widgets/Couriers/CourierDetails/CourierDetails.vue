<template>
    <div class="w-full flex flex-col shadow overflow-hidden rounded-lg">
        <div class="flex bg-white items-center gap-3 flex-row border rounded-t-lg p-5">
            <span class="text-lg font-medium">{{ courier.name }}</span>
            <div class="ml-auto">
                <Dialog>
                    ...
                    <DialogContent>
                        <DialogHeader>
                            <DialogTitle>Are you sure you want to delete this courier?</DialogTitle>
                        ...
                        <DialogFooter class="flex flex-row gap-2 justify-end">
                            ...
                            <button @click="deleteCourier" class="bg-primary font-medium text-white w-full whitespace-nowrap px-4 p-1 rounded shadow">Delete</button>
                        </DialogFooter>
                    </DialogContent>
                </Dialog>
            </div>
    ...
    <EditCourier ref="editForm" @updated="$emit('updated')" :courier="courier" v-model:isOpen="editForm.open" />
</template>
...
import { deleteCourier } from '@/services/orders';
import { toast } from 'vue-sonner';
import { getFileUrl } from '@/services/utils';

import EditCourier from '@/components/Widgets/Couriers/EditCourier/EditCourier.vue';
...
export default {
    ...
    methods: {
        ...
        async deleteCourier() {
            try {
                const deleted = await deleteCourier(this.courier.id);
                if (deleted) {
